          ...
          <template x-for="item in menus">
            <div>
              <template x-if="!item.isLabel">
                <li class="mt-2">
                  <a class="p-3" x-bind:class="pathname[1] === '' && item.name === 'dashboard' && 'active' || pathname[2] === item.name ? 'active' : ''" x-bind:href="item.link">
                    <span x-text="item.label">-</span>
                  </a>
                </li>
              </template>
              <template x-if="!!item.isLabel">
                <div class="p-2 ml-2" x-show="item.isLabel">
                  <span class="text-gray-500 text-base font-bold" x-text="item.label">
                  </span>
                </div>
              </template>
            </div>
...
<script>
  const datasidebar = [{
    ...
    },
    {
      ...
      name: "blogs",
      label: "Master Blog",
      link: "/admin/blogs?sortDirection=desc&sortColumn=id",
      isLabel: false,
    },
    {
      id: 7,
